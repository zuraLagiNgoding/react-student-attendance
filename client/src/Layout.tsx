    ...
    <div className="relative flex w-[100vw] h-screen overflow-x-hidden">
      ...
      <div className="relative lg:basis-5/6 bg-gray-100 sm:basis-[95%] h-screen flex flex-col min-h-screen max-h-screen max-w-full w-full sm:overflow-x-auto gap-10">
        <div className="z-[1] sticky top-0 flex bg-white border-b shadow-sm items-center justify-between sm:px-4 sm:my-0 p-5">
          ...
          <div
            className={clsx(
              "flex items-center justify-between md:gap-3 gap-0.5",
              !location.pathname.includes("inbox") ? "visible" : "invisible"
            )}
          >
            {/* <SearchBar/> */}
            <Link to="/inbox">
            ...
            <DropdownMenu
              ...
              onOpenChange={() => setOpenNotification(!openNotification)}
            >
              <DropdownMenuTrigger>
                <div
                  className="hover:bg-primary/10 p-1.5 rounded relative"
              ...
              >
                ...
                {notifications.length > 0 ? (
                  <DropdownMenuGroup>
                    {notifications.map((notification) => (
                      <DropdownMenuItem
                        className="gap-4"
                        onClick={() =>
                          navigate(`/inbox/${notification.message_id}`)
                        }
                      >
                        <Circle
                  ...
                  <DropdownMenuGroup className="w-full h-full text-center flex items-center justify-center">
                    <h1 className="mt-20 text-secondary/70">there are currently no incoming messages</h1>
                  </DropdownMenuGroup>
                )}
              </DropdownMenuContent>
            </DropdownMenu>
        ...
        <div className={clsx("px-4 pb-8 h-full overflow-hidden")}>
          <Outlet />
        </div>
      </div>
      <div
